...
template<typename T>
BigArray<T> *create(size_t max_count) {
  auto page_size = _virtual_page_size();
  size_t needed_reserved_size = sizeof(BigArray<T>) + max_count * sizeof(T);
  size_t needed_committed_size = sizeof(BigArray<T>);
...
template<typename T>
T* add_back(BigArray<T> *self, size_t amount) {
  self->count += amount;
  size_t needed_committed_size = sizeof(BigArray<T>) + self->count * sizeof(T);
...
template<typename T>
void remove_back(BigArray<T> *self, size_t amount) {
